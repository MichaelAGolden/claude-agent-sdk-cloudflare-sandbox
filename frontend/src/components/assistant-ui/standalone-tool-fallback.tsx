...
interface StandaloneToolFallbackProps {
  toolName: string;
  argsText: string;
  result?: any;
}

export const StandaloneToolFallback: FC<StandaloneToolFallbackProps> = ({
  toolName,
  argsText,
  result,
}) => {
      ...
      {!isCollapsed && (
        <div className="aui-tool-fallback-content flex flex-col gap-2 border-t pt-2">
          <div className="aui-tool-fallback-args-root px-4">
            <pre className="aui-tool-fallback-args-value whitespace-pre-wrap text-sm">
              {argsText}
            </pre>
          </div>
          {result !== undefined && (
